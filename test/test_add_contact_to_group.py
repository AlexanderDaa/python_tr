...
def test_add_contacts_to_group(app,orm):
    ...
    with pytest.allure.step('Get a non-empty group list without the contact %s' % contact_for_add):
    ...
    with pytest.allure.step('Get contact list for the group %s' % group_to_add):
    ...
    with pytest.allure.step('Add contact %s to the group %s' % (contact_for_add,group_to_add)):
