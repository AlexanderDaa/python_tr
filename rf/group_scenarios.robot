...
Modify group
    ...
    ${len}=  Get Length  ${old_list}
    ${index}=  Evaluate  random.randrange(${len})  random
    ${old_group}=  Get From List  ${old_list}  ${index}
    ${new_group}=  New GroupWithID  ${old_group}  name1ch  header1ch  footer1ch
    Modify Group  ${new_group}
    ${new_list}=  Get Group List
    Remove Values From List  ${old_list}  ${old_group}
    Insert Into List  ${old_list}  ${index}  ${new_group}
    Group Lists Should Be Equal  ${new_list}  ${old_list}
