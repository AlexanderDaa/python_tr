...
def change_group(app, random_group, new_group):
    app.group.change_group_by_id(random_group.id, new_group)

@then('the new list is equal to the old list with changed group')
def verify_group_changed(db,non_empty_group_list, random_group,new_group,app,check_ui):
    old_groups = non_empty_group_list
